...
DroidMemory::DroidMemory(const DroidMemory &droidmem) : Exp(droidmem.getExp()), Fingerprint(droidmem.getFingerprint())
{
}
...
DroidMemory &DroidMemory::operator=(const DroidMemory &droidmem)
{
    this->Exp = droidmem.getExp();
...
{
    return (Exp == droidmem.getExp() && Fingerprint == droidmem.getFingerprint());
}
