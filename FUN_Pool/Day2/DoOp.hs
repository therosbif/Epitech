...
doOp :: Maybe Int -> Char -> Maybe Int -> Maybe Int
doOp Nothing _ _ = Nothing
doOp _ _ Nothing = Nothing
doOp (Just x) '+' (Just y) = Just (x + y)
doOp (Just x) '-' (Just y) = Just (x - y)
doOp (Just x) '*' (Just y) = Just (x * y)
doOp (Just x) '/' (Just y) = x `safeDiv` y
doOp (Just x) '%' (Just y) = if y /= 0 then Just (x `mod` y) else Nothing
doOp _ _ _ = Nothing
...
main = do
  ...
  if length args /= 3 then exitWith (ExitFailure 84)
        ...
        let x = readInt (head args)
            y = readInt (args !! 2)
            op = if null (args !! 1) then ' ' else head (args !! 1)
            res = doOp x op y
         in printRes res)
